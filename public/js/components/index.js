        ...
        success: function (data) {
            ...
            if (to_search.length > 0) {
                if (obj.data.length > 0) {
                    ...
                    for (let j = 0; j < obj.data.length; j++) {
                        html += ' <div class="hover p-1 m-1" onmouseover="highlight_sug(this)" onmouseout="nohighlight_sug(this)" onclick="questions_modal(' + obj.data[j]['id'] + ')" onclick="add_vote(' + obj.data[j]['id'] + ')"><b>' + obj.data[j]['answers'] + ' (votes: ' + obj.data[j]['vote_count'] + ')</b></div>';
                    }
                } else {
                    html += '';
                }
